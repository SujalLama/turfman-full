  ...
  return (
    ...
    <div className="group mb-10 md:w-[calc(50%_-_2rem)] md:mx-4 bg-white shadow-lg rounded-[5px]">
        ...
        <div className="px-6 pt-3">
            <Link href={data.link} className="text-[13px]">
            ...
            {data.desc && <p className="mt-2 leading-snug">
                {data.desc}
            </p>}
            <div className="flex items-center justify-between mt-8 py-6 border-t ">
                {data.author && <div className="text-[13px]">					
                    <FaIcons icon="faUser" className="mr-1 text-primary" />
                    <span className=" tracking-[1px]">{data.author}</span>
                </div>}
                <div className="text-[13px]">	
                    <FaIcons icon="faComments" className="mr-1 text-primary" />
                    <span className="tracking-[1px]">{data.comments == 0 ? 'No' : data.comments} Comments</span>
                </div>            
            </div>
